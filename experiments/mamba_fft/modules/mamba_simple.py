...
class Mamba_simple(nn.Module):
    def __init__(self,
        ...
        self.blocks2 = nn.ModuleList(block_list2)
    ...
    def forward(self,x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        #     enc_out = (layer(enc_out) + enc_out)/2
        for i in range(self.layers):
            enc_out = (self.blocks[i](enc_out) + enc_out)/2
...
class Mamba_fft(nn.Module):
    def __init__(self, 
        ...
        self.projector_alpha = nn.Sequential(nn.Linear(configs.d_model, configs.d_model*2, bias=True),
                                            nn.SiLU(),
                                            nn.Linear(configs.d_model*2, configs.d_model*2, bias=True),
                                            nn.SiLU(),
                                            nn.Linear(configs.d_model*2, N_base, bias=True)
                                            )
        self.projector_A = nn.Sequential(nn.Linear(configs.d_model, configs.d_model*2, bias=True),
                                            nn.SiLU(),
                                            nn.Linear(configs.d_model*2, configs.d_model*2, bias=True),
                                            nn.SiLU(),
                                            nn.Linear(configs.d_model*2, N_base, bias=True)
                                            )
        self.projector_omega = nn.Sequential(nn.Linear(configs.d_model, configs.d_model*2, bias=True),
                                            nn.SiLU(),
                                            nn.Linear(configs.d_model*2, configs.d_model*2, bias=True),
                                            nn.SiLU(),
                                            nn.Linear(configs.d_model*2, N_base, bias=True)
                                            )
        self.projector_fai = nn.Sequential(nn.Linear(configs.d_model, configs.d_model*2, bias=True),
                                            nn.SiLU(),
                                            nn.Linear(configs.d_model*2, configs.d_model*2, bias=True),
                                            nn.SiLU(),
                                            nn.Linear(configs.d_model*2, N_base, bias=True)
                                            )
        # self.projector_alpha = nn.Sequential(nn.Linear(configs.d_model, N_base, bias=True))
        # self.projector_A = nn.Sequential(nn.Linear(configs.d_model, N_base, bias=True))
        # self.projector_omega = nn.Sequential(nn.Linear(configs.d_model, N_base, bias=True))
        # self.projector_fai = nn.Sequential(nn.Linear(configs.d_model, N_base, bias=True))
        self.projector_t = nn.Sequential(nn.Linear(1,10),
                                         nn.SiLU(),
                                         nn.Linear(10,1))
        self.projector = nn.Linear(configs.d_model, configs.pred_len, bias=True)
        self.configs = configs
    def forward(self,x_enc, x_mark_enc, x_dec, x_mark_dec):
        ...
        alpha = self.projector_alpha(enc_out_real).reshape(B,L,pred_len,-1)
        omega = self.projector_omega(enc_out_imag).reshape(B,L,pred_len,-1)
        fai = self.projector_fai(enc_out_imag).reshape(B,L,pred_len,-1)
...
class rbf(nn.Module):
    ...
    def RBF(self,x):
        ...
        if self.distance == 0:
            ...
            for i in range(l):
                K = torch.exp(-(x[:,i,:].unsqueeze(1)-x)**2/(2*(torch.pi)**2)).mean(-1)
                K_matrix.append(K.unsqueeze(1))
            K_matrix = torch.cat(K_matrix,dim=1)   
            K_matrix = torch.einsum('ijk->ikj',K_matrix)
            return K_matrix   
        else:
            return None
    ...
    def forward(self,x):
        ...
        K_matrix = self.RBF(x)
        if K_matrix !=None:
            x = torch.einsum('ijk,pkk->ijk',x,K_matrix)
            x = x/torch.sqrt(torch.tensor(K_matrix.shape[-1]).float())
...
class Mamba(nn.Module):
    def __init__(
        ...
        self.fourier_proj = nn.Linear(2,1)
        self.config = config
        self.porj_complex = nn.Linear(self.config.dec_in,self.config.dec_in*2)
    ...
    def forward(self, hidden_states, inference_params=None):
        ...
        """
        # if self.fft:
        #     hidden_states = torch.einsum('ijk->ikj',hidden_states)
        #     hidden_states = self.porj_complex(hidden_states)
        #     hidden_states = torch.einsum('ijk->ikj',hidden_states)
        """"
        hidden_states: (B, 2*L, D)
        real :hidden_states[:,:L,D]
        img :hidden_states[:,-L:,D]
        """
